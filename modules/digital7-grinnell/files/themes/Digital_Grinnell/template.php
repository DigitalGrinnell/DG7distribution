...
switch (Digital_Grinnell_art_get_drupal_major_version()) {
	case 5:
...
function Digital_Grinnell_art_get_drupal_major_version() {	
	$tok = strtok(VERSION, '.');
	//return first part of version number
	return (int)$tok[0];
}

function Digital_Grinnell_art_get_page_language($language) {
  if (Digital_Grinnell_art_get_drupal_major_version() >= 6) return $language->language;
  return $language;
}

function Digital_Grinnell_art_get_page_direction($language) {
...
function Digital_Grinnell_art_get_full_path_to_theme() {
  return base_path().path_to_theme();
}

function Digital_Grinnell_art_get_drupal_view() {
	if (Digital_Grinnell_art_get_drupal_major_version() == 7)
		return new Digital_Grinnell_art_view_drupal7();
	return new Digital_Grinnell_art_view_drupal56();
}

function Digital_Grinnell_art_export_version() {
	return 7;
}
...
class Digital_Grinnell_art_view_drupal56 {
...
?>
<!DOCTYPE html>
<html lang="<?php echo Digital_Grinnell_art_get_page_language($language); ?>" <?php echo Digital_Grinnell_art_get_page_direction($language); ?> >
<head>
  <?php echo $head; ?>
...
<div id="comments" class="art-comments">
	<div class="art-comment art-postcontent <?php print ($comment->new) ? ' comment-new' : ''; print ' '. $status; print ' '. $zebra; ?>">
		...
		<div class="art-comment-avatar">
			...
			else print '<img src="'.Digital_Grinnell_art_get_full_path_to_theme().'/images/no-avatar.jpg" alt="No Avatar">'; ?>
		</div>
...
<div id="comments">
  <?php print $content; ?>
	...
	function get_incorrect_version_message() {
		if (Digital_Grinnell_art_export_version() > 6) {
...
class Digital_Grinnell_art_view_drupal7 {
...
<?php endif; ?>
	...
	function get_incorrect_version_message() {
		if (Digital_Grinnell_art_export_version() < Digital_Grinnell_art_get_drupal_major_version()) {
