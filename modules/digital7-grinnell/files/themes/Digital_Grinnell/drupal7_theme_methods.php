...
$art_style = '';
$art_head = '';
function Digital_Grinnell_process_html(&$variables) {
	...
	$themePath = Digital_Grinnell_art_get_full_path_to_theme();
...
function Digital_Grinnell_breadcrumb($variables) {
 ...
 * @ingroup themeable
 */
function Digital_Grinnell_button($variables) {
 ...
 */
function Digital_Grinnell_preprocess_page(&$vars) {
  global $art_style, $art_head;
 ...
 * @ingroup themeable
 */
function Digital_Grinnell_menu_local_task($variables) {
 ...
 */
function Digital_Grinnell_feed_icon($variables) {
 ...
 * @ingroup themeable
 */
function Digital_Grinnell_node_preview($variables) {
  $node = $variables['node'];
 ...
 */
function Digital_Grinnell_art_links_woker_D7($content) {
  $result = '';
  ...
  foreach (array_keys($content['links']) as $name) {
	...
	if (isset($content['links'][$name]['#links'])) {
	  ...
	  if (is_array($links)) {
		$output = Digital_Grinnell_art_links_html_output_D7($links);
		if (!empty($output)) {
...
function Digital_Grinnell_art_terms_D7($content) {
	$result = NULL;
...
*/
function Digital_Grinnell_form_alter(&$form, &$form_state, $form_id) {
...
function Digital_Grinnell_art_label($variables) {
  $element = $variables['element'];
...
function Digital_Grinnell_form_element_label($variables) {
  $art_label = Digital_Grinnell_art_label($variables);
...
function Digital_Grinnell_form_element($variables) {
  $element = &$variables['element'];
  // This is also used in the installer, pre-database setup.
  $t = get_t();
...
function Digital_Grinnell_pager($variables) {
  $tags = $variables['tags'];
  $element = $variables['element'];
  ...
  if ($pager_total[$element] > 1) {
    ...
    return '<h2 class="element-invisible">' . t('Pages') . '</h2>' . Digital_Grinnell_art_item_list(array(
      'items' => $items,
...
function Digital_Grinnell_art_item_list($variables) {
  ...
  if (!empty($items)) {
    ...
    foreach ($items as $i => $item) {
      ...
      if (count($children) > 0) {
        // Render nested list.
        $data .= Digital_Grinnell_art_item_list(array('items' => $children, 'title' => NULL, 'type' => NULL, 'attributes' => $attributes));
      }
      if ($i == 0) {
