...
function Digital_Grinnell_art_node_worker($node) {
  $links_output = Digital_Grinnell_art_links_woker($node->links);
  $terms_output = Digital_Grinnell_art_terms_worker($node->taxonomy);
 ...
 * @ingroup themeable
 */
function Digital_Grinnell_art_terms_worker($terms) {
  $result = '';
 ...
 */
function Digital_Grinnell_art_links_woker($links, $attributes = array('class' => 'links')) {
  $output = '';
  ...
  if (!empty($links)) {
    ...
    foreach ($links as $key => $link) {
      ...
      $link_output = Digital_Grinnell_art_html_link_output($link);
      if (!empty($class)) {
...
function Digital_Grinnell_art_html_link_output($link) {
  $output = '';
  // Is the title HTML?
  ...
  if (isset($link['href'])) {
    if (Digital_Grinnell_art_get_drupal_major_version() == 5) {
...
function Digital_Grinnell_art_content_replace($content) {
...
function Digital_Grinnell_art_placeholders_output($var1, $var2, $var3, $id = '') {
...
function Digital_Grinnell_art_get_sidebar($sidebar, $vnavigation, $class) {
...
function Digital_Grinnell_art_submitted_worker($date, $author) {
  $output = '';
  if ($date != '') {
    ob_start();?>
...
function Digital_Grinnell_art_links_set($links) {
  $size = sizeof($links);
  if ($size == 0) {
    return FALSE;
  }
 ...
 *
*/
function Digital_Grinnell_art_node_title_output($title, $node_url, $page) {
  $output = '';
  if (!$page)
...
function Digital_Grinnell_art_process_menu_class($menu, $menu_class) {
  $result = $menu;
  $matches = array();
  $pattern = '~<ul(.*?class=[\'"])(.*?)([\'"])~';
...
function Digital_Grinnell_art_hmenu_output($content) {
  return Digital_Grinnell_art_process_menu_class($content, 'art-hmenu');
}

function Digital_Grinnell_art_vmenu_output($subject, $content) {
  if (empty($content))
    return;

  $result = '';
  $vmenu = Digital_Grinnell_art_process_menu_class($content, 'art-vmenu');
  $result .= <<< EOT
...
function Digital_Grinnell_art_replace_image_path($content) {
  $content = preg_replace_callback('/(src=)([\'"])(?:images[\/\\\]?)?(.*?)\2()/', 'Digital_Grinnell_art_real_path', $content);
  $content = preg_replace_callback('/(url\()([\'"])(?:images[\/\\\]?)?(.*?)\2(\))/', 'Digital_Grinnell_art_real_path', $content);
  return $content;
}

function Digital_Grinnell_art_real_path($match) {
  list($str, $start, $quote, $filename, $end) = $match;
  $full_path = Digital_Grinnell_art_get_full_path_to_theme().'/images';
